...
class Util
{
    ...
    {
        if ($time < 60) {
            return number_format($time, 2) . " " . Language::text('SECONDS', 'seconds');
        }
        $time = $time / 60;
        if ($time < 60) {
            return number_format($time, 2) . " " . Language::text('MINUTES', 'minutes');
        }
        $time = $time / 60;
        if ($time < 60) {
            return number_format($time, 2) . " " . Language::text('HOURS', 'hours');
        }
    ...
    public static function array_insert_after_key(&$array, $key, $element)
    {
        ...
        return self::array_insert($array, $position, $element);
    }
    ...
    public static function arrayToQueryString($data, $includeEmpty = false, $encodeFields = true)
    {
        ...
        if (is_array($data)) {
            foreach ($data as $k => $v) {
                ...
                if (($k == "") || ($v == "")) $skip = !$includeEmpty;

                if (!$skip) $returnValue .= (($encodeFields) ? urlencode($k) : $k) . "=" . (($encodeFields) ? urlencode($v) : $v) . "&";
            }
     ...
     * @return double
     */
    public static function getTimeDiferenceByInterval($interval, $dateStart, $dateEnd, $relative = false)
    {
